...
namespace BloomService.Web.Infrastructure.StorageProviders.AzureStorage
{
    public class AzureStorageProvider : IStorageProvider
    {
        ...
        public IStorageFile GetFile(string path)
        {
            ...
            return new AzureStorageFile(container.GetBlockBlobReference(path), this);
        }
        ...
        public IEnumerable<IStorageFile> ListFiles(string path)
        {
            ...
            var result = BlobClient
                ...
                .Select(blobItem => new AzureStorageFile(blobItem, this))
                .ToArray();
        ...
        public IEnumerable<IStorageFolder> ListFolders(string path)
        {
            ...
            if (path == String.Empty)
            {
                return container.ListBlobs()
                    ...
                    .Select<CloudBlobDirectory, IStorageFolder>(d => new AzureStorageFolder(d, this))
                    .ToList();
            }
        ...
        public void DeleteFolder(string path)
        {
            ...
            foreach (var blob in container.GetDirectoryReference(path).ListBlobs())
            {
                ...
                if (blob is CloudBlobDirectory)
                    DeleteFolder(new AzureStorageFolder((CloudBlobDirectory)blob, this).GetPath());
            }
        }
        ...
        public void RenameFolder(string path, string newPath)
        {
            ...
            foreach (var blob in container.GetDirectoryReference(path).ListBlobs())
            {
                if (blob is CloudBlockBlob)
                {
                    var azureBlob = new AzureStorageFile((CloudBlockBlob)blob, this);
                    RenameFile(azureBlob.GetPath(), Combine(fullNewPath, azureBlob.GetName()));
                }

                if (blob is CloudBlobDirectory)
                {
                    var azureFolder = new AzureStorageFolder((CloudBlobDirectory)blob, this);
